...
export const fetchCoursesInitial = createAsyncThunk('courses/fetchCoursesInitial', async () => {
  console.log('here')
  const response = await axios.get(`course/getall-courses`);
  return response.data;
});
...
const coursesSlice = createSlice({
  ...
  reducers: {
    ...
    addEnrollment(state, action) {
      ...
      if (enrollmentData?.role === "student") {
        const studentExists = state.enrollStudents.some(student => student.id === enrollmentData.id);
        if (!studentExists) {
          state.enrollStudents.push(enrollmentData);
        }
      } else if (enrollmentData?.role === "teacher") {
        const teacherExists = state.enrollTeachers.some(teacher => teacher.id === enrollmentData.id);
        if (!teacherExists) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCoursesInitial.pending, (state) => {
        state.loading = true;
        
        state.courses = [];
        state.error = null;
      })
      .addCase(fetchCoursesInitial.fulfilled, (state, action) => {
        state.courses = action.payload;
        state.loading = false;
      })
      .addCase(fetchCoursesInitial.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message;
      })
