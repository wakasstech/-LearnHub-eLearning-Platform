...
const sendEmailOnSuggestedFAQ = async (emailPayload) => {
    ...
    const htmlContentSuggestion = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>New FAQ Suggestion</title>
        <style>
            body {
    ...
    <body>
        <div class="container">
            <div class="message">
                ...
                <p>Dear Admin,</p>
                <p>A new FAQ suggestion has been submitted by <strong>${emailPayload.userFirstName} ${emailPayload.userLastName}</strong>. Please review and approve the suggestion if appropriate.</p>
                <ul>
                    ...
                    <li><strong>Suggested Question:</strong> ${emailPayload.suggestedQuestion}</li>
                    <li><strong>Suggested Answer:</strong> ${emailPayload.suggestedAnswer}</li>
                    <li><strong>References:</strong></li>
                    <ul>
                        ${Object.entries(emailPayload.references).map(
                          ([key, value]) => `<li>${key}: <a href="${value}" target="_blank">${value}</a></li>`
                        ).join('')}
                    </ul>
                </ul>
                <p>After approval, please ensure the FAQ is published and the user is notified accordingly.</p>
                <div class="note">
                    *This is an automated email. Please take the necessary action as soon as possible.
                </div>
            </div>
        </div>
    </body>
    ...
    let info = await transporter.sendMail({
      from: process.env.OUR_EMAIL1,
      to: 'hafiznizaqatali@gmail.com', //  admin's email
      subject: 'New FAQ Suggestion Submitted',
      html: htmlContentSuggestion,
    });
  
    if (info.messageId) {
      console.log(info, 84);
      return { success: true, message: 'Email sent for suggested FAQ to admin' };
    } else {
      return { success: false, message: 'Failed to send email for suggested FAQ to admin' };
    }
  };
...
const sendotp = async (req, res) => {
  ...
  const submitotp = async (req, res) => {
    try {
      const { otp, orderId, line_items,DiscountedAmount } = req.body;
